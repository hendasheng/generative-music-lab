...
```
谱面（秒级音符）→ 量化（八分网格，每步=该拍音符集合）→ 切分（32 步/乐句）
→ 马尔可夫链（学习乐句间转换）→ walk() 每次按概率选下一乐句
```

- **音符 100% 来自素材**（记忆），**顺序被概率重组**（重排）——听感是"同一首曲子每次演奏乐句顺序不同"
- 转移矩阵实时显示在页面上，可观察"当前乐句 → 下一乐句"的概率分布
- 换一版（新种子）改变行走路径；也可改 `buildMarkovChain` 里的平滑系数改变风格（重复率 / 跳变率）
