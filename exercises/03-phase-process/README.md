...
- 0.2 的 SVG 层级固定为：灰色轨道在底层，播放头位于轨道之上，彩色音符、音名与触发轮廓依次覆盖播放头；播放头只作为参照，不遮挡正在经过的音符
- 0.2 将 Moodist 背景 texture 映射为圆环之后的流动等高线：每条已加载素材生成五层半径错开的缓慢变形闭合轮廓，Pan 控制左右偏移，滤波频率控制轮廓细节；透明度读取实际音频淡入淡出包络，未播放时完全隐藏
- 控制 HUD 的“测试 Texture”按钮会按素材池顺序强制播放下一种环境音 12 秒，使用 1.2 秒淡入淡出，并把等高线可见度提高到正式状态的 4 倍，方便脱离概率调度逐项检查声音与视觉；需先点击播放并等待素材加载完成
- 用种子随机生成可复现的 loop 参数
...
- 等高线使用音频节点记录的 `visualStartAt / visualEndAt / visualFade`，由 `Tone.now()` 计算实际包络；不能仅根据 Transport 回调到达就提前显示。
- 每个已加载 texture 创建五层闭合轮廓。轮廓的左右中心取自 Pan，起伏复杂度取自低通滤波频率，动画相位随音频时间缓慢推进。
- 正式演出仍由概率调度决定 texture 是否出现；不活跃时分组透明度为 0。
- “测试 Texture”需要先完成正常播放的素材加载。之后每次点击按 Moodist 列表顺序强制播放一项 12 秒，淡入淡出为 1.2 秒，视觉强度为正式状态的四倍。
- 测试按钮只绕过出现概率，不绕过真实 Player、Gain 包络或等高线状态，因此可用于同时核对声音和视觉。
