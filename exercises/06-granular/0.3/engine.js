...
(() => {
  ...
  function fromXY(x,y) { const spray=(1-clamp(y,0,1))*.5; return {position:clamp(x,0,1),spray,reverse:reverseFromSpray(spray)}; }
  function toXY(params) { return {x:clamp(params.position,0,1),y:1-clamp(params.spray,0,.5)*2}; }
  window.Granular = { defaults, random, plan, demo, create, fromXY, toXY, reverseFromSpray };
})();
