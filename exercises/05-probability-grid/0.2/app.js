(() => {
  ...
  const state={bpm:120,root:9,scale:'minor',seed:'grid-05',tracks:[{cells:M.blank(),muted:false,sound:'reverie'},{cells:M.blank(),muted:false,sound:'machine'}]};
  ...
  state.tracks.forEach((track,trackIndex)=>{
    ...
    panel.innerHTML=`<div class="track-head"><h2>${String(trackIndex+1).padStart(2,'0')}</h2><button class="sound" aria-haspopup="dialog">${track.sound.toUpperCase()}</button><button class="mute" aria-pressed="false">静音</button><button class="random">随机</button><button class="clear">清空</button></div><div class="notes"></div><div class="grid" aria-label="轨道 ${trackIndex+1} 网格"></div>`;
    ...
    panel.querySelector('.sound').addEventListener('click',()=>openSounds(trackIndex));
  ...
  soundDialog.className='sound-dialog';soundDialog.setAttribute('aria-labelledby','sound-title');
  soundDialog.innerHTML='<div class="sound-heading"><h2 id="sound-title">Sound</h2><button class="close-sound" aria-label="关闭音色选择">×</button></div><div class="sound-options"></div>';
  document.body.append(soundDialog);
  soundDialog.querySelector('.close-sound').addEventListener('click',()=>soundDialog.close());
  soundDialog.addEventListener('click',event=>{if(event.target===soundDialog){const r=soundDialog.getBoundingClientRect();if(event.clientX<r.left||event.clientX>r.right||event.clientY<r.top||event.clientY>r.bottom)soundDialog.close();}});
  function openSounds(trackIndex) {
    $('sound-title').textContent='Sound · '+String(trackIndex+1).padStart(2,'0');
    const list=soundDialog.querySelector('.sound-options');list.replaceChildren();
    GridAudio.SOUNDS.forEach(sound=>{
      const button=document.createElement('button');button.className='sound-option';
      const selected=state.tracks[trackIndex].sound===sound.id;
      button.setAttribute('aria-pressed',selected);
      button.innerHTML='<span><strong></strong><small></small></span><span class="sound-check" aria-hidden="true"></span>';
      button.querySelector('strong').textContent=sound.label;
      button.querySelector('small').textContent=sound.hint;
      button.querySelector('.sound-check').textContent=selected?'✓':'';
      button.addEventListener('click',()=>{
        state.tracks[trackIndex].sound=sound.id;
        panels[trackIndex].querySelector('.sound').textContent=sound.label;
        soundDialog.close();
      });list.append(button);
    });
  ...
  document.addEventListener('keydown',event=>{
    if(soundDialog.open || event.code!=='Space' || event.repeat || event.composedPath().some(el=>['INPUT','SELECT','BUTTON','EXERCISE-CONTROLS'].includes(el.tagName)))return;
